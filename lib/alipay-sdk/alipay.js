...
class AlipaySdk {
    constructor(config) {
            ...
            if (config.alipayPublicKey) {
                ...
                //logger.info(config.alipayPublicKey);
            }
            ...
            this.sdkVersion = `alipay-sdk-nodejs-3.03`;
    ...
    exec(method, params = {}, option = {}) {
        ...
        //logger.info('[AlipaySdk]start exec, url: %s, method: %s, params: %s', url, method, JSON.stringify(execParams));
        return new Promise((resolve, reject) => {
            return resolve(url);//只产生签名字符串，结果给APP端使用
            config.urllib.request(url, {
                ...
                if (ret.status === 200) {
                     ...
                     * }
                     */
                    logger.info('[AlipaySdk]exec response: %s', ret.data);
                ...
                reject({ url, method, execParams: execParams, serverResult: ret, errorMessage: '[AlipaySdk]HTTP 请求错误' });
            })
                .catch((err) => {
                logger.error('[AlipaySdk]exec error: url=' + url + ", method=" + method + ", execParams:" + JSON.stringify(execParams));
                err.message = '[AlipaySdk]exec error: url=' + url + ", method=" + method + ", execParams:" + JSON.stringify(execParams);
                errorLog && errorLog(err);
                reject(err);
            });
        });
